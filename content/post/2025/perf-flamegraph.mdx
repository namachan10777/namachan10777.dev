...
![perf flamegraph](./perf/flamegraph.png "perf script report flamegraphで出力したFlameGraph。階層が表現されており、左上にズームボタンやリセットボタンがある。主に二つの山に分かれており、Threadで分けられていることが読み取れる")
...
![perf annotate](./perf/annotate.png "perf annotateの結果。movups命令が支配的なことが分かる")
...
![perf annotate (cache-miss)](./perf/cache-miss-rate.png "Slab::removeのcache-missの結果。全体に対して3%程度で、極端に高いわけでもない")

あれ？そこまで高わけでもなさそう。

![perf annotate (branch-miss)](./perf/branchmiss-focus.png "Slab::remove周辺の分岐予測ミスの結果")
