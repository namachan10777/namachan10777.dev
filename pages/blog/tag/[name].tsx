...
const Blog: React.FC<Props> = (props: Props) => {
    const og_image_url = `https://og-image-two-azure.vercel.app/${encodeURI(`#${props.tag}`)}.png?theme=dark&md=1&fontSize=100px`;
    return <div className="flex items-center flex-col w-screen">
        <Head>
            ...
            <meta property="og:url" content={`https://www.namachan10777.dev/blog/tag/${props.tag}`} />
...
export async function getStaticPaths() {
  ...
  const tags = new Set(Object.values(articles.blogs).map((article) => article.frontmatter.category).flat());
  return {
...
export async function getStaticProps(ctx: GetStaticPropsContext) {
    ...
    if (params) {
      return {
          props: {
              frontmatters: Object.values(articles.blogs).map(blog => blog.frontmatter).filter((frontmatter) => frontmatter.category.includes(params.name as string)),
              tag: params.name,
          }
      }
    }
