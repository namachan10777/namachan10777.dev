        ...
        <div className="w-full lg:w-1/2 p-5">
            ...
            <main>
                ...
                <ul className="pl-5 list-disc text-lg">
                    ...
                    <li key={frontmatter.date.toString()} className="underline text-gray-700 hover:text-black hover:font-medium text-lg" >
                        <Link href={`/diary/${frontmatter.date.toString()}`} passHref={true}>{frontmatter.date.toString()}</Link>
                    </li>)
                    }
                </ul>
            </main>
...
export async function getStaticProps() {
    ...
    return {
        props: {
            frontmatters: Object.values(articles.diaries).map(diary => diary.frontmatter)
        }
    }
}
